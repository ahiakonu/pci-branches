<x-app-layout>
    <div class="flex-shrink max-w-full px-4 w-full mb-6 my-3">
        <div class="flex-shrink max-w-full px-4 w-full md:flex md:justify-between">
            <p class="text-xl font-bold mt-0 mb-5">Divisional Reporting Preview</p>
            ...
            @endif
                ...
                <x-app-layout.input-div margin="mb-1 ">
                    <p class="text-indigo-600 ">
                        <x-app-svg.layout />
                        <strong>Divisional Report :: {{$report->report_month}}, {{$report->report_year}}</strong>
                    </p>
                </x-app-layout.input-div>

                <x-app-layout-form.gray-div>
                    {{--  --}}
                    <x-app-layout.input-div class=" sm:w-1/2 lg:w-1/2   ">
                        <x-app-layout-form.input input_type="text" name="report_year"
                            lablename="Reporting Year" :value="old('report_year',$report->report_year)" class="bg-gray-50" disabled  />
                    </x-app-layout.input-div>

                    <x-app-layout.input-div class=" sm:w-1/2 lg:w-1/2   ">
                        <x-app-layout-form.input input_type="text" name="report_month"
                            lablename="Reporting Month" :value="old('report_month',$report->report_month)" class="bg-gray-50" disabled  />
                    </x-app-layout.input-div>
                    ...
                    lablename="Did You Visit Any Branch ?" disabled>
                        @foreach ($yesno as $yn)
                            @if (old('visit_any_branch',$report->visit_any_branch) == $yn)
                    ...
                    <x-app-layout.input-div class=" sm:w-3/4 lg:w-3/4  ">
                        <x-app-layout-form.input input_type="text" name="branches_visisted"
                            lablename="If Yes, Name of Branch(es) Visisted" :value="old('branches_visisted',$report->branches_visisted)" />
                    </x-app-layout.input-div>
                    ...
                    <x-app-layout-form.input-select class=" sm:w-1/4 lg:w-1/4 " name="branches_paid_amalg"
                        ...
                        @foreach ($yesno as $yn)
                            @if (old('branches_paid_amalg',$report->branches_paid_amalg) == $yn)
                                <option value="{{ $yn }}" selected> {{ $yn }}
                                </option>
                            @else
                                <option value="{{ $yn }}"> {{ $yn }}
                    ...
                    <x-app-layout.input-div class=" sm:w-3/4 lg:w-3/4  ">
                        <x-app-layout-form.input input_type="text" name="branches_not_paid_amalg_details"
                            lablename="If No, Give Details" :value="old('branches_not_paid_amalg_details',$report->branches_not_paid_amalg_details)" input_required='false'/>
                    </x-app-layout.input-div>
                    ...
                    <x-app-layout-form.input-select class=" sm:w-1/4 lg:w-1/4 " name="sent_amalg"
                        ...
                        @foreach ($yesno as $yn)
                            @if (old('sent_amalg',$report->sent_amalg) == $yn)
                                <option value="{{ $yn }}" selected> {{ $yn }}
                                </option>
                            @else
                                <option value="{{ $yn }}"> {{ $yn }}
                    ...
                    <x-app-layout.input-div class=" sm:w-3/4 lg:w-3/4  ">
                        <x-app-layout-form.input input_type="text" name="not_sent_amalg_details"
                            lablename="If No, Why ?" :value="old('not_sent_amalg_details',$report->not_sent_amalg_details)" nput_required='false'/>
                    </x-app-layout.input-div>
                    ...
                    <x-app-layout-form.input-select class="sm:w-1/4 lg:w-1/4  " name="amalg_defaults"
                        ...
                        @foreach ($yesno as $yn)
                            @if (old('amalg_defaults',$report->amalg_defaults) == $yn)
                                <option value="{{ $yn }}" selected> {{ $yn }}
                                </option>
                            @else
                                <option value="{{ $yn }}"> {{ $yn }}
                    ...
                    <x-app-layout.input-div class=" sm:w-3/4 lg:w-3/4  ">
                        <x-app-layout-form.input input_type="text" name="amalg_defaults_branches"
                            lablename="If No, Which Branches ?" :value="old('amalg_defaults_branches',$report->amalg_defaults_branches)" nput_required='false'/>
                    </x-app-layout.input-div>
                    ...
                    <x-app-layout.input-div class="  ">
                        <x-app-layout-form.textarea name="amalg_defaults_action" lablename="What Action Was Taken ?"
                            ...
                            {{ old('amalg_defaults_action',$report->amalg_defaults_action) }}
                    ...
                    <x-app-layout-form.input-select class=" " name="compliance_issues"
                        ...
                        >
                        @foreach ($yesno as $yn)
                            @if (old('compliance_issues',$report->compliance_issues) == $yn)
                                <option value="{{ $yn }}" selected> {{ $yn }}
                                </option>
                            @else
                                <option value="{{ $yn }}"> {{ $yn }}
                    ...
                    <x-app-layout.input-div class="  ">
                        <x-app-layout-form.textarea name="compliance_issues_detalis" lablename="If Yes, Give Details"
                            input_required="false">
                            {{ old('compliance_issues_details',$report->compliance_issues_details) }}
                        </x-app-layout-form.textarea>
                    </x-app-layout.input-div>
                    ...
                    <x-app-layout-form.input-select class=" " name="divisional_programs"
                        ...
                        @foreach ($yesno as $yn)
                            @if (old('divisional_programs',$report->divisional_programs) == $yn)
                                <option value="{{ $yn }}" selected> {{ $yn }}
                                </option>
                            @else
                                <option value="{{ $yn }}"> {{ $yn }}
                    ...
                    <x-app-layout.input-div class="  ">
                        <x-app-layout-form.textarea name="divisional_ptogram_details" lablename="If Yes, Give Details"
                            input_required="false">
                            {{ old('divisional_ptogram_details',$report->divisional_ptogram_details) }}
                        </x-app-layout-form.textarea>
                    </x-app-layout.input-div>
                    ...
                    <x-app-layout-form.input-select class=" " name="all_branches_attended"
                        ...
                        @foreach ($yesno as $yn)
                            @if (old('all_branches_attended',$report->all_branches_attended) == $yn)
                                <option value="{{ $yn }}" selected> {{ $yn }}
                                </option>
                            @else
                                <option value="{{ $yn }}"> {{ $yn }}
                    ...
                    <x-app-layout.input-div class="  ">
                        <x-app-layout-form.textarea name="all_branches_attended_details" lablename="If Yes, Give Details"
                            input_required="false">
                            {{ old('all_branches_attended_details',$report->all_branches_attended_details) }}
                        </x-app-layout-form.textarea>
                    </x-app-layout.input-div>
