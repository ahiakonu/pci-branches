<x-app-layout>
    <div class="flex-shrink max-w-full px-4 w-full my-3">
        <div class="flex-shrink max-w-full px-4 w-full md:flex md:justify-between">
            <p class="text-xl font-bold mt-0 mb-5">Branch Properties</p>
        </div>
        <div class="p-6 bg-white dark:bg-gray-800 rounded-lg shadow-lg h-full">
            ...
            <div class="md:flex md:justify-between">
                    ...
                    <form>
                        ...
                        <div id="bulk-actions">
                            <label class="flex flex-wrap flex-row space-x-1">
                                <select id="report_status" name="report_status" onchange="HideButton('hide')"
                                    ...
                                    @foreach ($select_opt as $opt)
                                        @if (request()->get('report_status') == $opt)
                ...
                <div>
                    ...
                    @endphp
                    <form method="get" id="generateForm" action="/admin/reports/ajax/branchproperty">
                        <input type="hidden" name="down_division" value="{{ request()->get('division_id') }}">
                        <input type="hidden" name="down_church_status" value="{{ request()->get('report_status') }}">
            ...
            <x-app-layout-table.table class="hidden-header hidden-sort-after">
                <x-app-layout-table.thead>
                    ...
                    <x-app-layout-table.th lablename="photo1" />
                    <x-app-layout-table.th lablename="Division" />
                    <x-app-layout-table.th lablename="ChurchName" />
                    <x-app-layout-table.th lablename="PastorName" />
                    <x-app-layout-table.th lablename="MeetingPlace" />
                    <x-app-layout-table.th lablename="Own_Land" />
                    <x-app-layout-table.th lablename="Other_Lands" />
                    <x-app-layout-table.th lablename="Available_Docs" />
                    <x-app-layout-table.th lablename="Reg_Stage" />
                    <x-app-layout-table.th lablename="Doc_Location" />
                </x-app-layout-table.thead>
                        ...
                        @foreach ($reportdata as $branch)
                            <tr>
                                ...
                                <x-app-layout-table.td>
                                    @if ($branch->photo1 != null)
                                    <ul class="flex  items-center">
                                        <li class="inline-block">
                                            <a href="{{ asset($branch->photo1) }}" class="glightbox3"
                                        data-gallery="gallery{{$loop->iteration}}"><img src="{{ asset($branch->photo1) }}"
                                                    class="w-8 h-8 ltr:mr-1 rtl:ml-1 border border-dashed rounded-full"></a>
                                        </li>
                                        @if ($branch->photo2 != null)
                                            <li class="inline-block">
                                                <a href="{{ asset($branch->photo2) }}" class="glightbox3"
                                            data-gallery="gallery{{$loop->iteration}}"><img src="{{ asset($branch->photo2) }}"
                                                        class="w-8 h-8 ltr:mr-1 rtl:ml-1 border border-dashed rounded-full"></a>
                                            </li>
                                        @endif
                                ...
                                <x-app-layout-table.td> 
                                    {{ $branch->division_name }} </x-app-layout-table.td>
                                <x-app-layout-table.td> {{ $branch->church_name }} </x-app-layout-table.td>
                                <x-app-layout-table.td> {{ $branch->pastor_name }}</x-app-layout-table.td>
                                <x-app-layout-table.td> {{ $branch->meeting_place }} </x-app-layout-table.td>
                                <x-app-layout-table.td>{{ $branch->own_land }}</x-app-layout-table.td>
                                <x-app-layout-table.td>{{ $branch->other_lands }}</x-app-layout-table.td>
                                <x-app-layout-table.td>{{ $branch->available_doc }}</x-app-layout-table.td>
                                <x-app-layout-table.td> {{ $branch->registration_stage }}</x-app-layout-table.td>
                                <x-app-layout-table.td> {{ $branch->document_location }}</x-app-layout-table.td>
