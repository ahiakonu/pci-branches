<x-app-layout>
    <div class="flex flex-wrap flex-row">
        <div class="flex-shrink max-w-full px-4 w-full">
            <p class="text-xl font-bold mt-0 mb-5">Divisions</p>
        </div>
                ...
                <div class="flex flex-wrap flex-row -mx-4">
                    <div class="flex-shrink max-w-full px-4 w-full">
                        <div class="md:flex md:justify-between">
                            <x-app-layout-form.add-button href="{{ route('divisions.create') }}">Add Division
                        ...
                        <x-app-layout-table.table class="hidden-header hidden-sort-after">
                            <x-app-layout-table.thead>
                                <x-app-layout-table.th lablename="Division Name" />
                                <x-app-layout-table.th lablename="Divisional Leader" class="hidden lg:table-cell" />
                                {{-- <th class="hidden lg:table-cell" data-sortable="" style="width: 10.9187%;"><a href="#" class="dataTable-sorter">Update</a></th> --}}
                            ...
                            <x-app-layout-table.tbody>
                                @foreach ($divisions as $division)
                                    <tr>
                                        <x-app-layout-table.td> {{ $division->division_name }} </x-app-layout-table.td>

                                        <x-app-layout-table.td class="hidden lg:table-cell">
                                            {{ $division->divisional_leader }} </x-app-layout-table.td>

                                        <x-app-layout-table.td>{{ $division->country }}</x-app-layout-table.td>
                                            ...
                                            <div class="flex">
                                                @if ($division->id != 1)
                                                    <form action="{{ route('divisions.destroy', $division->id) }}"
                                                        method="post"
                                                        onsubmit="return SubmitDelete(this,'Delete {{ $division->division_name }}');">
                                                        @csrf
                                                        @method('delete')
                                                        <button type="submit"
                                                    ...
                                                    <a href="javascript:;"
                                                        onclick="SumitEdit('Edit {{ $division->division_name }} ?','/util/divisions/{{ $division->id }}/edit')"
